...
function getAccessToken(){
	$curlServiceUrl = $GLOBALS['PROJECT']['PAYPAL']['ENDPOINT'];
	$curlServiceUrl = $curlServiceUrl. "/v1/oauth2/token";
	$clientId = $GLOBALS['PROJECT']['PAYPAL']['CLIENT_ID'];
	$clientSecret = $GLOBALS['PROJECT']['PAYPAL']['CLIENT_SECRET'];
	$curlHeader = array(
		 "Content-type" => "application/json",
		 "Authorization: Basic ". base64_encode( $clientId .":". $clientSecret),
		 "PayPal-Partner-Attribution-Id" => $GLOBALS['PROJECT']['PAYPAL']['SBN_CODE']
		 );
	$postData = array(
...
function getApprovalURL($access_token, $postData){
	...
	$curlHeader = array("Content-Type:application/json", "Authorization:Bearer ".$access_token, "PayPal-Partner-Attribution-Id:" . $GLOBALS['PROJECT']['PAYPAL']['SBN_CODE']);
        //print_r($postData);
...
function doPayment($paymentID, $payerID, $transactionAmountArray){
    ...
    $curlHeader = array("Content-Type:application/json", "Authorization:Bearer ".$_SESSION['access_token'], "PayPal-Partner-Attribution-Id:" . $GLOBALS['PROJECT']['PAYPAL']['SBN_CODE']);
